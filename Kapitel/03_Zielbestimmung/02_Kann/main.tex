\section{Kann-Kriterien}

\begin{itemize}

    \item Hinzufügen von Etagenkarten über das %TODO:\Gls{Admin-Panel}END
    
    \item Optische Erkennung und automatisches Hinzufügen von Etagenkarten,
     welche von %TODO:\gls{Administratoren}END
     über das %TODO:\gls{Admin-Pannel}END
      hochgeladen werden.
    \item %TODO:\gls{Admin-Pannel}END
     bietet Funktionalität zum Aktualisieren/Ändern genutzter %TODO:\gls{APIs}END
      (beispielsweise %TODO:\gls{KIT}END
     -Seiten, die Informationen zu Gebäuden, Büros etc. zur Verfügung stellen)
    \item Standort teilen 
    \item Räume und Etagenkarten anderer %TODO:\gls{KIT}END
    -Gebäude anzeigen
    \item Finden von Hausmeisterbüros
    \item Finden von universitären Behörden
    \item Andere Anzeigensprachen in den Einstellungen festlegen
    \item Zusätzliche Informationen zu Räumen/Gebäuden anzeigen
    \item Anmeldung mittels %TODO:\gls{KIT}END
    -Account
    \item Namensvorschläge von Nutzern werden anderen Nutzern angezeigt. Bewerten genug Nutzer einen 
    Vorschlag positiv, so wird er den %TODO:\gls{Administratoren}END
     angezeigt.
    
        

\end{itemize}
